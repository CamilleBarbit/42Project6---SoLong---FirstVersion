...
/*   Updated: 2022/02/17 11:57:33 by cbarbit          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_exit_game(struc s_Map *map_ptr)
{
	...
	mlx_destroy_window(map_ptr->mlx_ptr, map_ptr->window_ptr);
	ft_free_map(map_ptr);	
}
...
{
	mlx_hook(game->mlx_win, 33, 1L << 5, ft_exit_game, map_ptr);
	mlx_hook(game->mlx_win, 2, 1L << 0, handle_key, map_ptr);
